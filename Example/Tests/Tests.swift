...
struct TestModel: Diffable, CustomDebugStringConvertible, Equatable {
...
final class MockedSimpleDataSource<Model: Diffable>: SimpleDataSource<Model> {
...
final class Tests: XCTestCase {
    ...
    static func syncSectionedDataSource(_ models: [TestModel]) -> MockedSectionDataSource<TestModel> {
        return MockedSectionDataSource(initialItems: models,
                                       sectionFunction: { model in [String(model.value.first ?? Character("1"))] },
    ...
    func testAsyncPerformance() {
        let oldArray = randomArray(length: 10000).map { TestModel(identifier: $0, value: String(randomNumber(0..<20))) }
        let newArray = randomArray(length: 10000).map { TestModel(identifier: $0, value: String(randomNumber(0..<20))) }
    ...
    func testSimpleUnsortedPerformance() {
        ...
        let oldArray = randomArray(length: 10000).map { TestModel(identifier: $0, value: String(randomNumber(0..<200))) }
        let newArray = randomArray(length: 10000).map { TestModel(identifier: $0, value: String(randomNumber(0..<200))) }
